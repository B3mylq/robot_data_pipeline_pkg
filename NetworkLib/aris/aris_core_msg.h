	...
	namespace core
	{
		...
		public:
			...
			*/
			...
			/** \brief Get the length of data including header
			*
			*/
		    auto totalSize() const -> std::int32_t;
			...
			*/
			...
			/** \brief set online_data_flag for control server to determine whether to send this message to the planner directly
			*
			*/
		    auto setOnlineDataFlag(bool flag) -> void;
			/** \brief get online_data_flag for control server to determine whether to send this message to the planner directly
			*
			*/
		    auto onlineDataFlag() const -> bool;
			/** \brief 获取Msg中的数据地址
			*
			*/
			...
			*/
			auto data()->char*;
			/** \brief Get the address of the whole message data including the header
			*
			*/
			auto fulldata() const -> const char*;
			/** \brief Get the address of the whole message data including the header
			*
			*/
			auto fulldata() -> char*;
		...
		public:
			...
			*/
			...
			virtual auto convertToZmqMsg(zmq::message_t &zmq_msg) const  -> void;

			virtual auto convertFromZmqMsg(const zmq::message_t &zmq_msg) -> void;
